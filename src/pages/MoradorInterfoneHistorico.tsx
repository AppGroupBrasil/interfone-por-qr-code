...
export default function MoradorInterfoneHistorico() {
    ...
    apiFetch("/api/interfone/calls")
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          const mine = data.filter((c: Call) => c.morador_id === user?.id || (user?.block && user?.unit && c.bloco === user.block && c.apartamento === user.unit));
          setCalls(mine);
        } else {
    ...
    <div className="min-h-dvh" style={{ background: "#f8fafc" }}>
      <header style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "16px 20px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "#64748b", margin: 0 }}>Suas chamadas do interfone</p>
        </div>
      </header>
      ...
      <main style={{ padding: "20px 16px 80px" }}>
        ...
        {!loading && !error && calls.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 20px", background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0" }}>
            ...
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#334155", marginBottom: 8 }}>Nenhuma chamada recebida ainda</h2>
            <p style={{ fontSize: 14, color: "#64748b", maxWidth: 320, margin: "0 auto" }}>
              Quando um visitante chamar você pelo interfone digital, a chamada aparecerá aqui.
            </p>
          </div>
        )}
              ...
              <div key={c.id} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 14 }}>
                ...
                {c.visitante_empresa && (
                  <div style={{ fontSize: 13, color: "#334155", marginBottom: 4 }}>
                    <strong>Empresa:</strong> {c.visitante_empresa}
                  </div>
                )}
